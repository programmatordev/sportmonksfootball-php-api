...
class StageEndpoint extends AbstractEndpoint
{
    use LanguageTrait;
    use SelectTrait;
    use IncludeTrait;
    ...
    public function getById(int $id): StageItem
    {
        $response = $this->sendRequest(
            method: 'GET',
            path: $this->formatPath('/v3/football/stages/{id}', [
                'id' => $id
            ])
        );

        return new StageItem($response);
    }
    ...
    public function getAllBySeasonId(int $seasonId): StageCollection
    {
        $response = $this->sendRequest(
            method: 'GET',
            path: $this->formatPath('/v3/football/stages/seasons/{seasonId}', [
                'seasonId' => $seasonId
            ])
        );
    ...
    ): StageCollection
    {
        ...
        $response = $this->sendRequest(
            method: 'GET',
            path: $this->formatPath('/v3/football/stages/search/{query}', [
                'query' => $query
            ]),
            query: [
                'page' => $page,
